...
let load_player_initialize = () => {
  ...
  <button class="input-button" id="player_input_button">look up</button>
    ...
    let player_input_button = document.getElementById("player_input_button");
    ...
    let load_high_score_data = () => {
        ...
        if (player_highscore_data) {
            player_highscore_data.forEach((score) => {
                if (!player_highscore_data)
                    return;
                if (player_highscore_data.indexOf(score) % 2 == 0)
                    table_string += "<tr>";
                ...
                if (player_highscore_data.indexOf(score) % 2 == 1)
                    table_string += "</tr>";
