...
let cooking_initialize = () => {
  ...
  <button id="cooking_input_button" class="input-button">calculate</button></div>`;
    let cooking_exp_input = document.getElementById("cooking_exp_input");
    let cooking_level_input = document.getElementById("cooking_level_input");
    let cooking_target_level_input = document.getElementById("cooking_target_level_input");
    let cooking_input_button = document.getElementById("cooking_input_button");
    ...
    cooking_input_button.addEventListener("click", () => {
        ...
        recipes.forEach((value) => {
            ...
            let required = calculate_xp_recursive((cooking_exp_input.value === "" ? total_exp_for_level(cooking_level_input.valueAsNumber) : cooking_exp_input.value.replace(/[,]/g, "")), cooking_target_level_input.valueAsNumber, returns);
            let required_string = "";
...
let calculate_xp_recursive = (starting_experience, target_level, returns) => {
    if (Number.isNaN(starting_experience) || Number.isNaN(target_level))
        return { number_required: NaN, number_produced: NaN };
