let farming_initialize = () => {
  ...
  let farming_farm_size_input = document.getElementById("farming_farm_size_input") as HTMLInputElement;
  let farming_input_button = document.getElementById("farming_input_button") as HTMLButtonElement;
  let farming_2x_input = document.getElementById("farming_2x_input") as HTMLInputElement;
  ...
  farming_input_button.addEventListener("click", () => {
    ...
    let xp_required = total_exp_for_level(farming_target_level_input.valueAsNumber) - (farming_exp_input.value === "" ? total_exp_for_level(farming_level_input.valueAsNumber) : farming_exp_input.value.replace(/[,]/g, "") as unknown as number);
    ...
    harvestable.forEach((value) => {
      ...
      item_string += "<td>" + returns[returns.length - 1].xp + "</td>"; let xp_per_tick = 0;
