let fletching_initialize = () => {
  ...
  <input type="number" placeholder="exp" value="" id="fletching_exp_input" style="margin: 10px;padding: 5px;width: 15%;box-sizing: border-box;border-radius: 6px;border: none;height: 24px;background-color: #373044ff;color: #dddce0;min-width:40px;" >
  <input type="number" placeholder="level" value="" id="fletching_level_input" style="margin: 10px;padding: 5px;width: 15%;box-sizing: border-box;border-radius: 6px;border: none;height: 24px;background-color: #373044ff;color: #dddce0;min-width:40px;"step = "1">
  <input type="number" placeholder="target level" value="" id="fletching_target_level_input" style="margin: 10px;padding: 5px;width: 15%;box-sizing: border-box;border-radius: 6px;border: none;height: 24px;background-color: #373044ff;color: #dddce0;min-width:40px;"step = "1">
    <label style="margin:10px;padding:5px;box-sizing:border-box;border:none;color:#dddce0">
  <input type="checkbox" id="fletching_2x_input" style="height:17px;width:17px;margin:0;vertical-align:middle;"><span style="vertical-align:middle;padding-left:5px;">2x</span></label>
    <label style="margin:10px;padding:5px;box-sizing:border-box;border:none;color:#dddce0">
  <input type="checkbox" id="fletching_arrow_input" style="height:17px;width:17px;margin:0;vertical-align:middle;"><span style="vertical-align:middle;padding-left:5px;">Arrows</span></label>
    <label style="margin:10px;padding:5px;box-sizing:border-box;border:none;color:#dddce0">  
  <input type="checkbox" id="fletching_anvil_input" style="height:17px;width:17px;margin:0;vertical-align:middle;"><span style="vertical-align:middle;padding-left:5px;">Anvil</span></label>
    <label style="margin:10px;padding:5px;box-sizing:border-box;border:none;color:#dddce0">
  <input type="checkbox" id="fletching_recycling_input" style="height:17px;width:17px;margin:0;vertical-align:middle;"><span style="vertical-align:middle;padding-left:5px;">recycling</span></label>
  <button id="fletching_input_button" style ="border-radius: 6px;height: 24px;margin-top: 10px;margin-bottom: 10px;border: none;vertical-align: top;background-color: #8777a3;color: #dddce0;padding: 5px;">calculate</button></div>`;
  ...
  let fletching_input_button = document.getElementById("fletching_input_button") as HTMLButtonElement;
  let fletching_2x_input = document.getElementById("fletching_2x_input") as HTMLInputElement;
  let fletching_recycling_input = document.getElementById("fletching_recycling_input") as HTMLInputElement;
  ...
  fletching_input_button.addEventListener("click", () => {
    ...
    let xp_required = total_exp_for_level(fletching_target_level_input.valueAsNumber) - (fletching_exp_input.value === "" ? total_exp_for_level(fletching_level_input.valueAsNumber) : fletching_exp_input.value.replace(/[,]/g, "") as unknown as number);
    ...
    if (fletching_arrow_input.checked) {
      let arrow_formulas: { product_id: number, level: number, requierments: { item_id: number, count: number; }[]; product_count: number; xp_per_craft: number; }[] = [];
      for (let index = 0; ; index++) {
        //@ts-ignore
        let value = FLETCHING_FORMULAS[index];
        if (typeof value === "undefined")
          break;
        let formula: { product_id: number, level: number, requierments: { item_id: number, count: number; }[], xp_per_craft: number, product_count: number; } = { product_id: 0, level: 0, requierments: [], xp_per_craft: 0, product_count: 0 };
        formula.product_id = value.item_id;
        formula.level = value.level;
        value.pattern.forEach((value: any) => {
          if (formula.requierments.findIndex((element) => { if (element.item_id === value) return true; }) === -1) {
            if (!(value === -1))
              formula.requierments.push({ item_id: value, count: 1 });
          }
          else {
            formula.requierments[formula.requierments.findIndex((element) => { if (element.item_id === value) return true; })].count += 1;
          }
        });
    ...
    if (fletching_anvil_input.checked) {
      let anvil_formulas: { product_id: number, level: number, chance: number, requierments: { item_id: number, count: number; }[], recycling_chance: number; }[] = [];
      for (let index = 0; ; index++) {
        //@ts-ignore
      ...
      let fletching_results: { icon_ids: number[], level: number, success: number, xp_per_success: number, xp_per_craft: number; number_required: { item_id: number, count: number; }[], number_produced: { item_id: number, count: number; }; }[] = [];
      if (!fletching_recycling_input.checked) {
        //items without recycling
        anvil_formulas.forEach((value) => {
          let fletching_result: { icon_ids: number[], level: number, success: number, xp_per_success: number, xp_per_craft: number; number_required: { item_id: number, count: number; }[], number_produced: { item_id: number, count: number; }; } = { icon_ids: [], level: 0, success: 0, xp_per_success: 0, xp_per_craft: 0, number_required: [], number_produced: { item_id: 0, count: 0 } };
